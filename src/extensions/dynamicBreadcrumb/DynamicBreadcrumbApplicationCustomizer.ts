...
export default class DynamicBreadcrumbApplicationCustomizer
  extends BaseApplicationCustomizer<IDynamicBreadcrumbApplicationCustomizerProperties> {
  ...
  private _configuration: IBreadcrumbConfiguration | undefined;
  ...
  public onInit(): Promise<void> {
    ...
    this._breadcrumbService = new BreadcrumbService(this.context.spHttpClient, webUrl);
  ...
  protected onDispose(): void {
    ...
    this.context.placeholderProvider.changedEvent.remove(this, this._onPlaceholdersChanged);
  ...
  private _loadBreadcrumb(forceConfigurationRefresh?: boolean): void {
    ...
    this._configurationService.getConfiguration(forceConfigurationRefresh)
      .then((configuration: IBreadcrumbConfiguration | undefined) => {
        ...
        if (!configuration) {
          this._items = [];
          this._statusMessage = this._canConfigure() ? strings.ConfigurationRequired : undefined;
        ...
        return this._configurationService.validateNavigationList(configuration.navigationListId)
          .then(() => this._breadcrumbService.getBreadcrumbForCurrentPage(configuration.navigationListId, this._locale))
      ...
      .catch((error: Error) => {
        ...
        this._items = [];
        this._statusMessage = this._canConfigure() ? strings.ConfigurationError : undefined;
        this._renderReactComponent();
      });
  }
  ...
  private _renderPlaceholder(): void {
    ...
    if (!this._topPlaceholder) {
      Log.warn(LOG_SOURCE, 'The Top placeholder is not available on this page.');
      return;
    }
  ...
  private _renderReactComponent(): void {
    ...
    const element: React.ReactElement<any> = React.createElement(DynamicBreadcrumb, {
      items: this._items,
      locale: this._locale,
  ...
  private _canConfigure(): boolean {
    return this.context.pageContext.web.permissions.hasPermission(SPPermission.manageLists);
  }
  ...
  private _onPlaceholderDisposed = (placeholderContent: PlaceholderContent): void => {
